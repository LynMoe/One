        ...
        if (fs.existsSync(DIR = DIR + namespace + '/') && namespace && fs.existsSync(DIR + 'settings.json')) {
            ...
            if (fs.existsSync(DIR) && fs.existsSync(DIR + 'info.json') &&
                ...
                if (/\bbr\b/.test(acceptEncoding)) {
                    ...
                    res.end(fs.readFileSync(DIR + 'file.br.data', {encoding: null}));
                } else if (/\bgzip\b/.test(acceptEncoding)) {
                    res.setHeader('Content-Encoding','gzip');
                    res.end(fs.readFileSync(DIR + 'file.gz.data', {encoding: null}));
                } else {
                    res.setHeader('Content-Encoding','deflate');
                    res.end(fs.readFileSync(DIR + 'file.data', {encoding: null}));
                }
